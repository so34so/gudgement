...
def compare_to_member(month_overconsumption, year, month):
    # 이번 달이면
    ...
    # 아니면
        ...
        sql = 'SELECT * FROM chart WHERE month_overconsumption IS NOT NULL AND year = %s AND month = %s'
        df = pd.read_sql_query(sql, engine, params=(year, month))
...
# virtual_id로 멤버 구하기
def get_member_by_virtual_id(virtual_id, year, month):
    sql = 'SELECT * FROM chart WHERE virtual_account_id = %s AND year = %s AND month = %s'
    df = pd.read_sql_query(sql, engine, params=(virtual_id, year, month))
