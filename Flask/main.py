...
@app.route("/<int:year>/<int:month>/<int:virtualId>/<int:month_overconsumption>", methods=['POST'])
def month_transaction(virtualId, year, month, month_overconsumption):
    ...
    if month_overconsumption != -1:
        ...
        for key, value in member_dict.items():
            try:
                ...
                month_overconsumption_member[key] = round((member_month_transaction / db.month_overconsumption_rate(key)) * 100, 1)
        ...
        if (total_amount != 0):
            this_month_rate = round((total_amount / month_overconsumption) * 100, 1)
    ...
    if (last_month_overcomsumption != None):
        last_month_rate = round((last_month_transaction / last_month_overcomsumption) * 100, 1)
